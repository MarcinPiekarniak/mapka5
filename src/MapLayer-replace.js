...
class MapLayerGeojson extends GeoJsonLayer {
	static layerName = 'MapLayer'

	constructor(props) {
    super({
      id: 'airport-geojson',
      data: props.data/*airportGeojson*/,
      opacity: 1,
      stroked: false,
      filled: true,
      extruded: false,
    ...
    getElevation: f => props.elevation || 0,
      ...
      getLineDashArray: f => {
        let dashedIDs = [5, 8,9,23];
        if (dashedIDs.includes(extractId(f.properties.id))) {
          return [20,8];
        }
        console.log('in dash');
        console.log(f);
...
//let layers = [new MapLayerGeojson(), new BuildingGeojson()];
